...
from utilities import check_valid_array # helper_functions
...
def plot_decision_boundaries(X, y, classifier,
   ...
   legend : int (default: 1)
    ...
    check_valid_array(X, y, y_int=True)  # Validate X and y arrays
    dim = X.shape[1]
    ...
    Z = classifier.predict(X_predict.astype(X.dtype))
    Z = Z.reshape(xx.shape)
    # Plot decision region
    # Make sure contourf_kwargs has backwards compatible defaults
    contourf_kwargs_default = {'alpha': 0.35, 'antialiased': True}
    ...
    # print score mean classifier score accuracy
    score = classifier.score(X,y)
    ax.text(xx.max() - .3, yy.min() + .3, 
           ('mean acc : %.3f' % score).lstrip('0'),
